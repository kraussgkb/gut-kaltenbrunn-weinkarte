...
/* WINEFUNDAY_REQUEST_STORM_FIX_V1 — protected; restores uploaded ID images while limiting each wine to one automatic request */
(()=>{'use strict';
const preferredSrc=w=>{const explicit=String((w&&w.image)||'').trim();return explicit||('Weinbilder/'+encodeURIComponent((w&&w.id)||'')+'.jpg')};
