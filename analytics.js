...
/* WINEFUNDAY_REQUEST_STORM_FIX_V1 — protected; prevents thousands of empty/fallback image requests */
(()=>{'use strict';
function protectRenderers(){
  if(typeof window.card==='function'&&!window.card.__wfRequestFix){
    ...
    const wrapped=function(w){
      const html=originalCard(w);
      if(String((w&&w.image)||'').trim())return html;
      return html.replace(/<div class="pic"><img src=""[^>]*><\/div>/,'<div class="pic missing"></div>');
    };
  ...
  if(typeof window.detail==='function'&&!window.detail.__wfRequestFix){
    ...
    const wrapped=function(w){
      const html=originalDetail(w);
      if(String((w&&w.image)||'').trim())return html;
      return html.replace(/<div class="bottle"><img src=""[^>]*><\/div>/,'<div class="bottle missing" data-wf-wine-id="'+encodeURIComponent((w&&w.id)||'')+'"></div>');
    };
    wrapped.__wfRequestFix=true;
...
function resolveDetailBottle(){
  const box=document.querySelector('.bottle[data-wf-wine-id]');
  if(!box||box.dataset.wfResolved==='1')return;
  box.dataset.wfResolved='1';
  const id=new URLSearchParams(location.search).get('id')||decodeURIComponent(box.dataset.wfWineId||'');
  if(!id)return;
  const candidates=['webp','jpeg','jpg','png'].map(ext=>'Weinbilder/'+encodeURIComponent(id)+'.'+ext);
  let i=0;
  const next=()=>{
    if(i>=candidates.length)return;
    const src=candidates[i++],probe=new Image();
    probe.onload=()=>{
      const img=document.createElement('img');
      img.src=src;img.alt='Flaschenbild zum aktuellen Wein';
      img.onerror=()=>{img.remove();box.classList.add('missing')};
      box.classList.remove('missing');box.appendChild(img);
    };
    probe.onerror=next;
    probe.src=src;
  };
...
document.addEventListener('DOMContentLoaded',()=>setTimeout(resolveDetailBottle,0));
const detailObserver=new MutationObserver(()=>resolveDetailBottle());
detailObserver.observe(document.documentElement,{childList:true,subtree:true});
setTimeout(()=>detailObserver.disconnect(),5000);
})();
...
function wire(){
  ...
  ['webp','jpeg','jpg','png'].forEach(ext=>candidates.push('Weinbilder/'+encodeURIComponent(id)+'.'+ext));
  let i=0;
  const tryNext=()=>{
